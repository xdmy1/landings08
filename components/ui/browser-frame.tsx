...
export function BrowserFrame({ domain, className = "", children }: { domain?: string, className?: string, children: React.ReactNode }) {
  return (
    <div className={`border border-divider/60 shadow-[0_30px_70px_-30px_rgba(0,0,0,0.65)] ${className}`} style={{ background: '#211A13' }}>
      <div className="flex items-center gap-1.5 px-3.5 py-2.5 border-b border-divider/40" style={{ background: '#261F17' }}>
        ...
        {domain && (
          <span className="ml-3 inline-flex items-center gap-1.5 text-[10px] font-mono tracking-wide text-ink-light px-2.5 py-0.5 truncate" style={{ background: '#2E251C' }}>
